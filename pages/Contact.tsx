    ...
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-slate-900 mb-4">Centre d'Aide & Contact</h1>
          <p className="text-slate-600 text-lg max-w-2xl mx-auto">
              Une urgence ? Une question sur votre contrat ? 
              Notre équipe basée à Rabat est là pour vous accompagner 7j/7.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-1 space-y-6">
             <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 text-center hover:shadow-md transition group">
                ...
                <h3 className="font-bold text-slate-900 text-lg">Service Client</h3>
                <div className="flex items-center justify-center gap-2 text-slate-500 text-sm mt-1 mb-3">
                    <Clock size={14}/> Lundi - Dimanche, 8h - 22h
                </div>
                <p className="text-blue-600 font-bold text-xl tracking-wide">+212 5 37 10 20 30</p>
                <p className="text-xs text-slate-400 mt-2">(Prix d'un appel local)</p>
             </div>
             ...
             <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 text-center hover:shadow-md transition group">
                ...
                <p className="text-slate-500 text-sm mt-1 mb-3">Réponse garantie sous 24h</p>
             ...
             <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 text-center hover:shadow-md transition group">
                ...
                <h3 className="font-bold text-slate-900 text-lg">Siège Social</h3>
                <p className="text-slate-500 text-sm mt-1">Technopolis, Bâtiment B2</p>
                <p className="text-slate-900 font-bold">11100, Sala Al Jadida - Rabat</p>
             </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
             ...
             <div className="bg-white rounded-2xl shadow-lg border border-blue-100 overflow-hidden">
                 <div className="bg-slate-900 p-4 text-white flex items-center justify-between">
                     <div className="flex items-center gap-3">
                         ...
                         <div>
                             <h3 className="font-bold">Assistant de Résolution</h3>
                             <p className="text-xs text-slate-300">Réponses immédiates aux problèmes courants</p>
                         </div>
                     </div>
                 </div>
                 ...
                 <div className="h-64 bg-slate-50 p-6 overflow-y-auto space-y-4">
                     {chatMessages.map((msg, i) => (
                         <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                             <div className={`max-w-[80%] p-3 rounded-xl text-sm ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-700 rounded-tl-none shadow-sm'}`}>
                                 {msg.text}
                             </div>
                         </div>
                     ))}
                 ...
                 <div className="p-4 bg-white border-t border-slate-100">
                     <p className="text-xs font-bold text-slate-500 mb-3 uppercase">Urgence & Sécurité :</p>
                     <div className="flex gap-2 overflow-x-auto pb-2 mb-4 no-scrollbar">
                         <button onClick={() => handleQuickAction("Problème de Paiement")} className="whitespace-nowrap px-3 py-2 bg-red-50 text-red-600 rounded-lg text-xs font-bold border border-red-100 hover:bg-red-100 flex items-center gap-2">
                             <AlertTriangle size={14}/> Paiement bloqué
                         </button>
                         ...
                         <button onClick={() => handleQuickAction("Question Contrat")} className="whitespace-nowrap px-3 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold border border-blue-100 hover:bg-blue-100 flex items-center gap-2">
                             <CreditCard size={14}/> Contrat & Légalisation
                         </button>
                     </div>
             ...
             <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                <h2 className="text-xl font-bold text-slate-900 mb-6">Formulaire de Contact</h2>
                <form className="space-y-6">
                   <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Nom complet</label>
                        <input type="text" className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Votre nom" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
                        <input type="email" className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="votre@email.com" />
                      </div>
                   </div>
                   <div>
                   ...
                   <div>
                      ...
                      <textarea rows={5} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Détaillez votre demande ici..."></textarea>
                   </div>
                   <button className="w-full py-4 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition shadow-lg flex items-center justify-center gap-2">
                     Envoyer la demande <ChevronRight size={20}/>
                   </button>
                </form>
             </div>
